const summaryCalc = (
	...
	if (Number(data.distancia) <= truckRadio && !futuroPrice) {
		...
		if (servicesTypeCk.PE && Number(servicesType.PE) > 500) vars.peaje = Number(servicesType.PE) - 500;
	...
	if (futuroBasico && Number(data.distancia) <= 50) return 0;
